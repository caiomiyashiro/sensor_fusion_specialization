...
int main ()
{
	...
	// viewer->addPointCloud<pcl::PointXYZ> (cloud, color, "sample_cloud");
	// viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 10, "sample_cloud");
	
  	std::cout << "Test Search" << std::endl;
	// std::vector<int> test;
	// for(int i=0;i<points.size();i++){
	// 	test  = tree->search(points[i],3.0);
	...
	std::vector<Color> colors = {Color(1,0,0), Color(0,1,0), Color(0,0,1)};
  	for(std::vector<int> cluster : clusters)
  	{
		...
		viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 10, "cluster"+std::to_string(clusterId));
  		++clusterId;
  	}
  	if(clusters.size()==0)
