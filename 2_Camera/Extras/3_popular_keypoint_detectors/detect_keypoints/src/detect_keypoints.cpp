...
void detKeypoints1()
{
    ...
    detector->detect(imgGray, points_of_interest);
    t2 = ((double)cv::getTickCount() - t2) / cv::getTickFrequency();
    cout << "FastFeatureDetector with n= " << points_of_interest.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;

    cv::Mat visImage2 = img.clone();
    cv::drawKeypoints(img, points_of_interest, visImage2, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    string windowName2 = "FastFeatureDetector";
